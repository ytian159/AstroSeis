function [face,numface,ds,xs0,ys0,zs0,thetas,phis,height,V,Tri]=gen_mesh_elp_topo(Rm,n,ee)
...
a=sqrt(Rm^2/ee);
b=sqrt(Rm^2*ee);
xj=b*sin(theta).*cos(phi);
yj=b*sin(theta).*sin(phi);
zj=a*cos(theta);
hh=sqrt(xj.^2+yj.^2+zj.^2)-Rm;
%The ana
%hh(indtheta)=-Rm*0.1;
%r0 = 10.993e3;
r0=Rm;
...
%     yj = r*sin(theta(j))*sin(phi(j));
    V(j, 1:3) = [xj(j) yj(j) zj(j)]; % vertex with topography
end
