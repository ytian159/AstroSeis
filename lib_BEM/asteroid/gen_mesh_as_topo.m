function [face,numface,ds,xs0,ys0,zs0,thetas,phis,height,V,Tri]=gen_mesh_as_topo(Rm,n,nfold,theta_cut)
...
%theta_cut=pi/4;
hh=zeros(length(V),1);
indtheta=theta<theta_cut;
theta_sel=theta(indtheta);
hh=real(hh*nfold);
hh(indtheta)=-Rm*(1-cos(theta_cut)./cos(theta_sel));
%hh(indtheta)=-Rm*0.1;
%r0 = 10.993e3;
r0=Rm;
for j =1: length(V)
