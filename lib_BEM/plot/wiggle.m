function wiggle(x, y, data, sc_fac, pos_neg,nfold)
...
if (nargin==5) [ny,nx] = size(data); end
...
if (nx<ny)
    ...
    for (ii=1:nx)
        ...
        hold on;
        %plot(ones(1,length(y)).*x(ii)-1,y,'k',data(:,ii).*sc_fac+x(ii),y,'b');
        plot(data(:,ii).*sc_fac+x(ii),y,'b');
    ...
    for (ii=1:ny)
        ...
        %set(handle_fill,'LineStyle','none');
        hold on
        %plot(x,ones(1,length(x)).*y(ii)-1,'k',x,data(ii,:).*sc_fac+y(ii),'b');
        plot(x,data(ii,:).*sc_fac+y(ii),'b');
    end
end
hold off;
axis tight;
