function [TRAC2]=cal_G_st(face1,face2,w,lamda,mu,rho,Q,nint,nxi)
...
if ngd1==ngd2
    ...
    parfor i=1:ngd2
        ...
        ys=ps(2);
        zs=ps(3);
        STs(:,:,i)= int_self_G_tri_vec (vp,vs,rho,w,...
            xs,ys,zs,face2(i),wis,refs,ridfac);
    end
    
%% Overall integral and constructing matrix

    TRAC2=zeros(3*ngd1,3*ngd2);
    for j=1:ngd1
        ps=face2(j).ic;
        xs=ps(1);
        ys=ps(2);
        zs=ps(3);
        ST0=G_tri_int_vec(vp,vs,rho,w,xia,yia,zia,wia,xs,ys,zs,area,STs(:,:,j),j);
        TRAC2(j,:)=ST0(:,1);
        TRAC2(j+ngd1,:)=ST0(:,2);
        TRAC2(j+2*ngd1,:)=ST0(:,3);
    end
else
   TRAC2=zeros(3*ngd1,3*ngd2);
    for j=1:ngd1
        ...
        ys=ps(2);
        zs=ps(3);
        ST0=G_tri_int_vec_diflay(vp,vs,rho,w,xia,yia,zia,wia,xs,ys,zs,area);
        TRAC2(j,:)=ST0(:,1);
        TRAC2(j+ngd1,:)=ST0(:,2);
        TRAC2(j+2*ngd1,:)=ST0(:,3);
    end 
