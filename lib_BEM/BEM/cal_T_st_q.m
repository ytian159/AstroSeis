function [TRAC]=cal_T_st_q(face1,face2,w,lamda,mu,rho,Q,nint,nxi)
...
if ngd1==ngd2
    ...
    parfor i=1:ngd2
        ...
        STs(:,:,i)= int_self_trac_tri_vec (vp,vs,rho,w,...
            xs,ys,zs,face2(i),wis,refs,ridfac)+eye(3);
    end
    
%% Overall integral and constructing matrix

    TRAC1=zeros(ngd1,3*ngd2);
    TRAC2=zeros(ngd1,3*ngd2);
    TRAC3=zeros(ngd1,3*ngd2);
    parfor j=1:ngd1
        ps=face2(j).ic;
        xs=ps(1);
        ys=ps(2);
    ...
    parfor j=1:ngd1
        ...
        ys=ps(2);
        zs=ps(3);
        ST0=trac_tri_int_vec_diflay(vp,vs,rho,w,xia,yia,zia,wia,xs,ys,zs,area,n1a,n2a,n3a,nsub);
        ST0=ST0.';
        TRAC1(j,:)=ST0(1,:);
        TRAC2(j,:)=ST0(2,:);
        TRAC3(j,:)=ST0(3,:);
